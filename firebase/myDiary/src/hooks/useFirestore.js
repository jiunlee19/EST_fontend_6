...
// Timestamp config에 작성해서 import해서 쓰는게 맞지만,
// 이미 addDoc, collection을 직접 불러오고 있고,
// useFirestore에서만 쓰이기 때문에 여기서 불러오겠습니다.
...
export const useFirestore = (transaction) => {
    ...
    // collection() : 이미 있는 파일이라면 주소를 불러오고, 없다면 테이블을 생성합니다.
    ...
    const addDocument = async (doc) => {
        try {
            const createdTime = Timestamp.fromDate(new Date()); // 시간 정보
            const docRef = await addDoc(colRef, { ...doc, createdTime });
            // addDoc return : 도큐먼트 참조 정보

            dispatch({ type: "addDoc", payload: docRef });
            // initState의 document에 docRef를 담아줍니다.
        } catch (err) {
            dispatch({ type: "error", payload: err.messagge });
        }
    };
